...
class EC2Manager:
    ...
    def __init__(self, region: Optional[str] = None):
        ...
        try:
            ...
            error_msg = (
                ...
                "  2. AWS credentials file: ~/.aws/credentials (run 'aws configure')\n"
    ...
    def get_instance_info(self, instance_id: str) -> Dict:
        ...
        try:
            ...
            if error_code == "AuthFailure" or "credentials" in error_message or "unable to locate credentials" in error_message:
                error_msg = (
                    "AWS authentication failed. Please verify your AWS credentials:\n"
                    "  1. Check environment variables: AWS_ACCESS_KEY_ID and AWS_SECRET_ACCESS_KEY\n"
                    "  2. Verify ~/.aws/credentials file (run 'aws configure')\n"
                    "  3. Ensure credentials have EC2 permissions\n"
                    f"Error: {e}"
                )
                logger.error(error_msg)
                raise EC2ManagerError(error_msg) from e
            ...
            # Catch any other exceptions that might be credential-related
            error_message = str(e).lower()
