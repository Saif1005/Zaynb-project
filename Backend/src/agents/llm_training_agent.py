...
class LLMTrainingAgent(BaseAgent):
    ...
    def validate_input(self, context: Dict[str, Any]) -> bool:
        """Validate input context (variants ou JSON métriques VCF)."""
        if "vcf_metrics" in context or "vcf_metrics_path" in context:
            return True
        if "variants" in context or "pathogenic_cancer_variants" in context:
            return True
        self.logger.warning("No variants or vcf_metrics in context, skipping LLM training")
        return False
    ...
    def execute(self, context: Dict[str, Any]) -> AgentResult:
        ...
        vcf_metrics = context.get("vcf_metrics")
        vcf_metrics_path = context.get("vcf_metrics_path")
        
        # Check if fine-tuning is requested
        should_train = context.get("train_llm", False) or self.config.get("auto_train", False)
        
        try:
            # Prepare training data : priorité au JSON métriques (output pipeline GATK/VCF → bioLLM)
            self.logger.info("Preparing training data...")
            if vcf_metrics is not None:
                training_example = self.data_prep.prepare_from_metrics_json(
                    vcf_metrics, analysis_result=context.get("prediction_results")
                )
            elif vcf_metrics_path and Path(vcf_metrics_path).exists():
                training_example = self.data_prep.prepare_from_metrics_json(
                    vcf_metrics_path, analysis_result=context.get("prediction_results")
                )
            else:
            ...
            if should_train:
                ...
                if not instance_id or not ssh_key:
                    ...
                    model_name = context.get("model_name") or self.config.get("model_name")
